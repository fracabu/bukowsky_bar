<template>
  <nav class="navbar fixed top-0 left-0 right-0 z-50 bg-black/95 backdrop-blur-sm border-b border-white/10">
    <div class="nav-container max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
      ...
      <!-- Mobile Menu Toggle -->
      <button
        class="menu-toggle lg:hidden flex flex-col gap-1.5 w-8 h-8 items-center justify-center"
        @click="toggleMenu"
      ...
      <!-- Mobile Menu -->
      <div
        class="mobile-menu fixed top-20 left-0 right-0 bg-black/98 backdrop-blur-md lg:hidden transition-all duration-300 overflow-hidden border-b border-white/10"
        :class="isMenuOpen ? 'max-h-screen py-8' : 'max-h-0 py-0'">
        <ul class="flex flex-col gap-6 px-6 list-none">
          <li v-for="link in navLinks" :key="link.to">
            <router-link
              :to="link.to"
              @click="closeMenu"
